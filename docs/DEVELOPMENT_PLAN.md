...
Covered by `tests/test_command_run.py` (repository CRUD + list/prune against real Postgres) and `tests/test_async_command_executor.py::TestDBBackedCrossInstancePoll` (launches a real command with `info`, evicts the local registry entry to simulate a different instance, confirms `poll_command` still returns correct status/output/exit_code from the DB — and that omitting `info` preserves the old `not_found` behavior).
