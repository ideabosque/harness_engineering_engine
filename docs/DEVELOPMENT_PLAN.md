...
> **Date:** 2026-09-12
> **Status:** P1-P6 built and wired end to end; CLI package manager fully implemented (registration + lazy install, not a stub); skill deployment sources directly from git — no S3 or other artifact store; P8 (recursive discovery + CLI package auto-register) done; P9 (`reference_files` + OpenAI-assisted section generation) done and verified live against real repos; §18 integration gaps (async `runCommand`/`pollCommand`, non-blocking on-demand refresh, fuzzy `searchSkills`, output-truncation byte counts, DB-backed multi-instance command-run registry, lazy skill/CLI-package install with git-commit-aware reinstall) resolved
...
| P8 | Recursive `SKILL.md` discovery; `cli_packages` auto-register at deploy time, lazy install on first `runCommand` | Done, see Known issue #6 (fixed) and §18 G-7 (lazy-install redesign, 2026-09-12) |
...
6. **Fixed.** `deploySkillPackage` could not fully onboard a skill that (a) nested `SKILL.md` more than one directory deep in its repo, and/or (b) declared a `cli_packages` dependency that had never been separately registered or installed. Raised while evaluating a real external repo shaped this way (`multilingual_slide_video_production_system`). Discovery was `content_root.glob("*/SKILL.md")` plus a root-level fallback — no recursive search, so a `SKILL.md` nested under a further subfolder (e.g. `src/skills/<name>/SKILL.md`) failed with "No SKILL.md found." Separately, a skill's `cli_packages` frontmatter was purely declarative: it was read only at `runCommand` time, and `ensure_package()` required the package to already exist via a prior `insertUpdateCliPackage` call — no path registered or installed a declared `cli_packages` entry automatically. **Applied:** discovery is now recursive (`content_root.rglob("SKILL.md")`, pruning `.git`, `handlers/skill_deployment.py:164-170`); a new `_ensure_cli_packages()` helper runs per discovered skill, right before that skill's DB row is written — it auto-registers (`register_cli_package`) any `cli_packages` entry that carries `git_repository_url`+`version` (entries without those are assumed pre-registered), then (as of 2026-09-04) always called `ensure_package()` to install/verify immediately at deploy time. A failed install raised, landing that skill in `failed` rather than registering a skill whose declared dependency doesn't actually work — the skill row was never written in that case. `runCommand`'s existing `ensure_package()` call was unchanged and acted as a cheap safety-net re-check.
   **Superseded 2026-09-12 (see §18 G-7):** the eager-install-at-deploy-time decision described above has been reversed. `_ensure_cli_packages()` now only registers; `ensure_package()` install/verify is lazy, deferred to first `runCommand`, and a broken dependency surfaces there instead of failing the deploy. Registration behavior, recursive discovery, and the regression-guard coverage described here are otherwise unchanged. Covered by `tests/test_skill_deployment.py::TestDeploySkillPackageDiscoveryAndCliPackages` (nested discovery, register-without-install, a broken package no longer blocking deploy, and a no-op regression guard for skills with no `cli_packages`).
...
| `cli_packages` | No | External Python CLI dependencies. An entry with `git_repository_url`+`version` is auto-registered (DB bookkeeping only) at deploy time (P8); one without them is assumed pre-registered via `insertUpdateCliPackage`. Installation itself is lazy — deferred to first `runCommand` against the skill, which also detects and reinstalls on git-commit drift, not just a changed `version` string (§18 G-7). |
...
deploySkillPackage(source, gitRef?, version?, skill_name?)
  ...
  -> cache this version's content locally, keyed by (skill_name, version) —
     active or not, deploy never writes into HSK_SKILL_ROOT itself (§18 G-7)
...
`deploySkillPackage` auto-activates a skill's first-ever version (`is_active=true`, `deployment_status="deployed"`), so a brand-new skill is registered and discoverable immediately after deploy. Every version after that still lands inactive (`is_active=false`, `deployment_status="registered"`) and requires an explicit `promoteSkillVersion` before `skill(name)`, `run_command`, or `refreshLocalSkills` will see it (see Known issue #2 above, fixed). Either way, deploy itself only caches the content locally (keyed by `(skill_name, version)`) — it never installs into the live `HSK_SKILL_ROOT/<name>` directory. That install is lazy: the first `skill()`/`run_command` call against this version, on whichever instance receives it, installs synchronously from this local cache when present (no network) or by cloning from git when not (§18 G-7). Promoting a cached version later is still an instant local swap either way, not a fresh fetch.
...
`SKILL.md` discovery is recursive — a repo can nest a skill any number of directories deep (e.g. a monorepo shaped like `src/skills/<name>/SKILL.md`), not just directly under the repo root. A skill declaring `cli_packages` in its frontmatter has each entry auto-registered (when it carries `git_repository_url`+`version`) as part of this same deploy — registration only, no install. Installation is lazy, deferred to first `run_command` against the skill; a broken/uninstallable dependency surfaces there instead of failing the deploy (see §18 G-7, superseding Known issue #6's original eager-install decision).
...
### Python CLI package dependencies (delivered — P8 registration, §18 G-7 lazy install)

> **Stale note removed 2026-09-12:** this subsection originally described `handlers/cli_package_manager.py` as a v1.1 stub (`ensure_package()` raising `NotImplementedError`). That was already out of date by the time P8 shipped (2026-09-05) — `ensure_package` has been fully implemented since, called both at deploy time (superseded 2026-09-12, see §18 G-7) and on every `runCommand`. The design below is current, with one addition: `ensure_package` also compares a locally-tracked "last installed commit" against the registered `git_ref`'s current commit (a cheap `git ls-remote`), not just the `version` string — so a floating ref (e.g. `main`) that gains new commits without a version bump is still detected and reinstalled (§18 G-7).
...
- `deploySkillPackage` auto-registers (via `register_cli_package`) any `cli_packages` a deployed skill declares in its frontmatter, using the package name/git URL/version/`git_ref` already present there.
- **Superseded 2026-09-12 (§18 G-7):** as originally delivered, `deploySkillPackage` also auto-*installed* (via `ensure_package`) each registered package immediately, and a failed install failed that skill's deployment entry. Installation is now lazy — deferred to first `runCommand` — so a broken dependency no longer blocks deploy; see G-7 for the current behavior.
- `runCommand`'s existing `ensure_package()` call is left in place — it's now the *only* place installation happens, not a safety-net re-check on top of an eager deploy-time install. It no-ops immediately when already installed at the registered version and commit.

Exit criteria (met, as originally scoped):

- A single `deploySkillPackage` call against a repo with a nested `SKILL.md` and a `cli_packages` entry results in: the skill registered and (if first version) active; the declared CLI package registered. (Superseded: "installed/verified" and "no install-on-first-use delay" — see G-7, which deliberately reverses this.)
- A CLI package install failure during deploy is reported per-skill in `failed`, not raised as an unhandled exception that aborts the rest of a multi-skill deploy. (Superseded: install no longer happens at deploy time at all, so this scenario cannot occur there — see G-7.)
...
| Discovery + CLI package registration (lazy install) | `SKILL.md` nested several directories deep is still discovered; a declared `cli_packages` entry is auto-registered at deploy time but not installed; a broken/uninstallable package no longer blocks deploy; a skill with no `cli_packages` never touches the CLI package manager. | Done (`test_skill_deployment.py::TestDeploySkillPackageDiscoveryAndCliPackages`, §18 G-7) |
...
### G-7 (Resolved 2026-09-12) — Skill content and CLI package installation were eager at deploy time; neither is needed until first use

Both were installed synchronously as part of `deploySkillPackage` (skill content into the live `HSK_SKILL_ROOT/<name>` directory when a version became active; each declared `cli_packages` entry `pip install`ed and verified via `ensure_package`), rather than on first actual use. Two problems, one for each:

- **CLI packages:** a real `pip install` (network + build) ran on every deploy, and a broken/uninstallable dependency failed the *entire skill's deploy* (Known issue #6, decided 2026-09-04) — a skill with valid `SKILL.md` but one bad dependency never got registered at all. Separately, `ensure_package`'s staleness check compared only the registered `version` **string** against the installed pip distribution version — a `cli_packages` entry pinned to a floating `git_ref` (e.g. `main`) could gain new commits with no version bump, and `ensure_package` would never notice, forever serving a stale install.
- **Skill content:** cheap by comparison (a local copy from the same clone already fetched for registration, no network) — but in a multi-instance deployment, eager-install-at-deploy only ever pre-warms whichever single instance happened to receive the `deploySkillPackage` call. Every other instance already relied entirely on the existing on-demand refresh (G-2) to install on first request. Keeping deploy eager on just one instance made that one instance's behavior inconsistent with the rest of the fleet for no real benefit once more than one instance is in play.

**Applied:**

1. **Skill content is now lazy on every instance uniformly.** `deploy_skill_package` still calls `skill_version_cache.store_version(...)` unconditionally (so `promoteSkillVersion`/`rollbackSkill` stay instant local swaps) but no longer calls `install_from_cache`/`write_local_metadata` for the active version — deploy never writes into the live skill directory itself. `skill_reader.skill()`'s existing on-demand-refresh check (G-2) now tries a **synchronous, local-only fast path first**: `skill_version_cache.install_from_cache(skill_root, name, version)` — a plain local copy, no network — before falling back to the background-thread git-clone path. Since `store_version` always ran at deploy, whichever instance deployed (or redeployed, or promoted) a version installs it instantly and synchronously on its own first `skill()`/`run_command` call, with no `"refreshing"` placeholder; only an instance that never cached that version locally falls through to the pre-existing background-refresh-from-git behavior. `promoteSkillVersion`/`rollbackSkill`'s own eager `install_from_cache` call (`mutations/skill_management.py::_activate_local_content`) is unchanged — that's a deliberate, explicit "make this version live now" admin action, not a deploy-time default.
2. **CLI packages are registered but not installed at deploy.** `_ensure_cli_packages()` in `skill_deployment.py` now only calls `register_cli_package` — pure DB bookkeeping. `ensure_package()` (the actual `pip install`) is unchanged in its own call sites (`command_executor.py`, `mutations/skill_management.py::RunCommand._run_background`) and now runs *only* there, on first `runCommand` against the skill. A broken dependency surfaces as a `runCommand` error on first use instead of failing the deploy.
3. **`ensure_package` now also tracks the installed git commit, not just the version string.** A host-local marker file (`HSK_SKILL_ROOT/.hsk-cli-packages/<package_name>.json` — host-local by design, like a skill's own `.hsk-skill.json`, since "what's actually installed on this host's Python environment" can't be represented safely as shared DB state in a multi-instance deployment) records the commit last installed from. Each `ensure_package` call cheaply resolves the registered `git_ref`'s current commit (`git_client.resolve_ref_sha` — a `git ls-remote`, no clone) and reinstalls if either the version string OR the resolved commit has drifted from what's recorded — catching the floating-ref case Known issue #6's original check missed. A resolution failure (offline) falls back to the version-string-only check rather than forcing a spurious reinstall; a version match with no marker yet (a package installed before this tracking existed) is trusted as-is and the marker is backfilled for the next check.

Covered by `tests/test_skill_deployment.py` (deploy caches content under `.hsk-versions/<name>/<version>/` but never touches the live skill directory; `cli_packages` registered without `ensure_package` ever being called; a broken package's `ensure_package` failure no longer blocks deploy), `tests/test_cli_package.py::TestEnsurePackageCommitStaleness` (matching version+commit skips reinstall; commit drift alone — unchanged version string — still triggers reinstall; an absent marker with an already-matching version is trusted and backfilled, not force-reinstalled).

**End-to-end testing note (2026-09-12):** ran the full lazy-install flow against the real `git@github.com:abacusipllc/ingredient_optimization_agent` repo (deploy → confirm no live directories exist yet → first `skill()` read installs synchronously from the local version cache with no network → a simulated second instance with an empty skill root correctly falls back to a real background git clone). That last step surfaced a genuine, pre-existing bug, unmasked by G-7 rather than caused by it: `skill_refresh.py::_resolve_skill_content_dir` only ever checked `<clone>/<skill_name>/SKILL.md` or `<clone>/SKILL.md` — this repo nests every skill two directories deeper (`.claude/skills/<name>/SKILL.md`), a shape `deploy_skill_package`'s own recursive `rglob` discovery already handled fine, but `refresh_single_skill` did not. The skill deployed successfully but could never be refreshed on any host that didn't already have that exact version locally cached — previously a narrow edge case (only mattered for a genuinely different host, or `refreshLocalSkills`), but G-7 made every instance without a local cache hit depend on this path, so it needed fixing as part of this work rather than filed separately. **Fixed:** `_resolve_skill_content_dir` now mirrors deploy's own recursive-search-plus-frontmatter-name-match instead of assuming a fixed folder shape. Covered by `tests/test_skill_refresh.py::TestResolveSkillContentDirNestedLayout` (a nested `.claude/skills/<name>/SKILL.md` layout refreshes correctly; requesting a name absent from the repo still raises clearly rather than installing the wrong skill).
